...
**Important — file progress signaling:** When you finish presenting a file (or group), call **devdocket-signalPhase** with the current \`phase\` and a \`filePath\` for the exact file path from the diff. When you are presenting the **last file** (or last group) in the reading order, signal \`phase: "lastFile"\` instead of \`phase: "walkthrough"\`. DevDocket also uses \`filePath\` to verify when the final file has been presented, so always include it for file walkthrough phases.
