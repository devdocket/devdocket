...
- **devdocket-signalPhase** — **Call this at the end of every response** to signal the current walkthrough phase. Pass \`phase: "summary"\` after presenting the opening overview, \`phase: "walkthrough"\` during the file-by-file presentation, \`phase: "lastFile"\` when presenting the **last file** in the reading order, or \`phase: "wrapup"\` after the final wrap-up. When presenting a single file, also pass \`filePath\` with the exact relative path from the diff. When presenting a group, pass \`filePaths\` with every exact relative path in that group. DevDocket uses those paths to verify whether the "Next file" button should be shown.
...
**Important — file progress signaling:** When you finish presenting a file, call **devdocket-signalPhase** with the current \`phase\` and a \`filePath\` for the exact file path from the diff. When you finish presenting a group, pass \`filePaths\` with every exact file path in that group. When you are presenting the **last file** (or last group) in the reading order, signal \`phase: "lastFile"\` instead of \`phase: "walkthrough"\`. DevDocket also uses these paths to verify when the final file has been presented, so always include them for file walkthrough phases.
