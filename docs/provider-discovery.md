...
4. **Item already dismissed** — Dismissed items never reappear. Check the Sources tab to see all unfiltered items the provider knows about, regardless of inbox state. Repositories excluded by `devDocketGithub.filteredRepos` are excluded from Sources too.
