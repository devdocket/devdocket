...
- **Event-Driven Architecture**: Core state changes generally follow mutate → save → fire. When a work item moves to a new state, EventEmitter callbacks drive UI tree refreshes. This decoupling keeps services independent and testable without implying provider refreshes are triggered by state-store events.
- **Provider Items as References**: `discovered-state.json` persists lightweight discovered state: `inboxState` plus optional version markers such as `version`/`resurfaceVersion`. Item fields (for example title, description, and url) are held in the `ProviderRegistry`'s in-memory cache and refreshed from providers, which keeps persisted state minimal while allowing item data to stay current.
...
- **Code review fix**: Preserved write-after-persist pattern by computing sortOrder with temporary state mutation, then restoring original state before store.save(). This ensures in-memory map stays consistent if save throws.
