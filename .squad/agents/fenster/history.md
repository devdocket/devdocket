...
- **WorkItem metadata fields:** Added optional `branchName?: string` and `worktreePath?: string` to `WorkItem` interface. These fields persist the git state created by the Start Git Work action.
...
- **StartWorkAction integration:** Modified `StartWorkAction.run()` to call `devdocket.updateMetadata` after successfully creating the worktree and branch. Stores both `branchName` and `worktreePath` on the WorkItem.
- **Cleanup service:** Created `packages/core/src/services/gitCleanup.ts` with `promptGitCleanup()` function. Checks if worktree directory exists and if branch exists in the parent repo. Derives repo path from worktree path using pattern `{repo}-issue{N}` → `{repo}`.
...
- **Key pattern:** Metadata fields on WorkItem are optional and action-specific. Core doesn't interpret them — actions own their lifecycle.
