...
- **Configuration namespacing**: Each action has its own config section (`workcenterAiReview` vs `workcenterAiWalkthrough`) with independent `customPromptPath` settings in `package.json`.
